...
with app.app_context():
	class User(WithTemplates, HasRoutes, Model):
		...
		routes = [
			...
			{
				'route': '/<ObjectId:_id>/maps',
				'view_function': 'maps_view',
				'methods': ['GET']
			}
		]
		...
		@classmethod
		def maps_view(cls, _id):
			limit = int(request.args.get('limit', 0))
			skip = int(request.args.get('skip', 0))

			return cls._format_response(Map.list({'user_id': _id}, limit=limit, skip=skip))
		...
		def tagged_view(cls, tag):
			...
			return cls._format_response({
				'tag': tag,
				'users': cls.list({'tags': tag}, limit=limit, skip=skip)
			})
